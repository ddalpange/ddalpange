...
- 기존에도 Atomic Class 기반으로 개발되고 있어 방식에 더 효율적인 개발을 위해 Vanila Extract를 tailwind로 전환
- 에이전트를 활용해 스타일 회귀를 유닛테스트, ee 테스트, iOS Simulator를 활용한 A-A 테스트, 조금의 검수를 통해 Zero QA I
ssue로 전환
- datadog, loki 기반의 실시간 모니터링 로그, 릴리즈 버전 관리
- 피드 목록이 연속으로 쌓이는 UX여서 윈도잉, 렌더링 최적화로 웹뷰 메모리가 200kb 언더로 관리되도록 개선
- 아파트 입주민 외 외부 유저들도 받을 수 있는 오픈 커뮤니티 개발
- 에이전트 병렬 개발 환경에서 손쉬운 QA를 위해 기기에 등록된 사내망 고정 ip에 도메인을 할당받고 Portless를 활용해 각각의 영
속적이고 독립된 도메인으로 띄울 수 있게끔 셋업
