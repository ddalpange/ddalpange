...
- 중고차 경매 웹 전체 구현
    - 10가지 이상의 복잡한 경매 상태 관리
    - 20가지 이상의 페이지 개발
- 상태-페이지 격리 아키텍처 설계
    - 각 상태와 페이지를 명확히 격리하여 가독성 확보
    - 유지보수 용이한 구조로 후속 개발자 온보딩 비용 절감
- UI 컴포넌트 시스템 구축
    - 다수 UI의 컴포넌트화로 재사용성 확보
    - 디자이너-엔지니어 협업 효율화를 위한 컴포넌트 설계 고민
- 연관 기존 코드 대규모 리팩토링
