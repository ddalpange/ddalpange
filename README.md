...
> 당근내 버티컬 사업팀에서 중고차 시장의 1등을 지향하며, 직거래 기반 서비스의 1→10 성장을 이끌었습니다. “거래가 늘려면 고객의 거래비용(노력)이 줄어야 한다”는 가설 아래, 빠르고 공격적인 실험 문화 속에서 편의성/신뢰를 만드는 제품을 반복적으로 만들었습니다.  대표적으로 전문가 기반 가격 추천, 차량 감정(검수) 실험, 그리고 딜러·탁송까지 포함한 복잡도 높은 거래 플로우를 설계·구현했습니다.  그 결과(지표는 보안상 근사치) MAU 500만+, 월 게시글 1만5천+, 월 거래완료 1만+ 규모로 성장에 기여했고, 나아가 완전 비대면 C2B 내차팔기를 0→1로 구현해 1년 만에 거래전환율 20%+, 월 경매 1만+, 딜러 4천+를 만들었습니다. 고객과 시장에 관심을 가지고 제품을 만드는 메이커형 엔지니어입니다.
